...
def move_indexer_to_slot(slot: int):
  ...
  send_grbl_gcode_cmd(absolute_mode + " " + cm_mode + " " + G01_linear_interpolation + " " + indexer_axis + batt_slot_abs_loc_cm[slot] + " " + indexer_feed_rate)
...
def ESTOP():
  ...
  # send_grbl_gcode_cmd("!")
  send_grbl_gcode_cmd_bytes(grbl_soft_reset_cmd) # this is ctrl-x (soft-reset)
  SERIAL_CONNECTION.flushInput()
